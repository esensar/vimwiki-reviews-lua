...
function M.get_review_index(vimwiki_index)
	...
	local entries = scandir.scan_dir(reviews_path, {
		hidden = false,
		add_dirs = false,
	...
	for _, entry in pairs(entries) do
		...
		if string.match(filename, "^%d*-year" .. ext .. "$") then
			local year = string.match(filename, "(%d*)-year")
			if index[year] == nil then
				index[year] = {
					months = {},
				}
			end
			index[year].year = filename
		elseif string.match(filename, "^%d*-%d*-month" .. ext .. "$") then
			...
			if index[year].months[month] == nil then
				index[year].months[month] = {
					weeks = {},
				}
			end
			index[year].months[month].month = filename
		elseif string.match(filename, "^%d*-%d*-%d*-week" .. ext .. "$") then
...
function M.open_vimwiki_review_index(vimwiki_index)
	...
	for _, year in pairs(years) do
		...
		-- Sort months
		local months = {}
		for k in pairs(index[year].months) do
			table.insert(months, k)
		end
		...
		for _, month in pairs(months) do
			...
			table.insert(lines, "")
			if index[year].months[month].month then
